...
func main() {
	...
	if dbPath == "" {
		...
		if appData != "" {
			dbPath = filepath.Join(appData, "DiscordStorageEngine", "drive.db")
		} else {
	...
	if err := database.ResetIncompleteState(); err != nil {
	...
	if *passwordFlag != "" {
		...
		if passHash != "" && passHash != computedHash {
			log.Fatalf("that password does not match your saved master password")
		}
		if passHash == "" {
			_ = database.SetSetting("master_pass_hash", computedHash)
		}
		masterKey = key
		fmt.Println(" [OK] Password checked and ready to go.")
	} else if !*noPromptFlag && isConsole(os.Stdin.Fd()) {
		if passHash != "" {
			for attempts := 1; attempts <= 3; attempts++ {
				pass, err := readPasswordConsole(fmt.Sprintf(" [LOCK] Type your master password (try %d of 3): ", attempts))
				if err != nil || pass == "" {
					fmt.Println(" [!] Nothing typed. Starting locked so you can unlock in your browser.")
					break
				}
				key := crypto.DeriveKey(pass, salt)
				if crypto.ComputeSHA256(key) == passHash {
					masterKey = key
					fmt.Println(" [OK] Password correct. Drive is now unlocked.")
					break
				}
				fmt.Println(" [X] Wrong password, try again.")
			}
		} else {
			fmt.Println(" [SETUP] Pick a master password to protect your cloud storage:")
			pass1, err1 := readPasswordConsole(" > Type your password: ")
			if err1 == nil && pass1 != "" {
				pass2, err2 := readPasswordConsole(" > Type it again to confirm: ")
				if err2 == nil && pass1 == pass2 {
					...
					fmt.Println(" [OK] All set. Master password saved and drive unlocked.")
				} else {
					fmt.Println(" [!] Passwords did not match. You can set it in your browser instead.")
				}
			} else {
				fmt.Println(" [i] Skipped. You can set your password inside the web app.")
			}
		}
	}
	...
	catalogManager := catalog.NewSyncManager(database, discordClient, masterKey)
	...
	log.Printf(" App is ready. Open your browser at: %s\n", serverURL)
