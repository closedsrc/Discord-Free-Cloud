...
func (e *Engine) broadcastTelemetry(event TelemetryEvent) {
	e.mu.RLock()
...
func (e *Engine) UploadFile(ctx context.Context, localFilePath, virtualPath, parentID string, targetGuildIDs ...string) (string, error) {
	...
	for _, t := range targets {
		...
		if err != nil || len(storageChannels) == 0 {
			...
			if t.GuildID != "" && bToken != "" {
				res, setupErr := e.discord.AutoSetupServerWithToken(ctx, bToken, t.GuildID)
	...
	log.Printf("[UPLOAD START] %s | Size: %.2f MB | Parts: %d | Servers: %d", fileName, float64(fileSize)/(1024*1024), totalChunks, len(serverChannelSets))

	go e.runUploadPipeline(jobCtx, uploadTracker, jobRec, fileRec, normPath, workItems, key)

	return jobID, nil
}

func (e *Engine) runUploadPipeline(ctx context.Context, tracker *activeUpload, job *db.JobRecord, file *db.FileRecord, normPath string, items []partWorkItem, key []byte) {
	defer func() {
		e.mu.Lock()
	...
	if err != nil {
		...
		log.Printf("[UPLOAD ERROR] Could not read file %s: %v", normPath, err)
		e.broadcastTelemetry(TelemetryEvent{
			JobID:      job.ID,
			FileID:     file.ID,
			...
			for {
				...
				case item, ok := <-chunkChan:
					...
					if err != nil && err != io.EOF && n == 0 {
						atomic.AddInt32(&tracker.activeWorkers, -1)
						log.Printf("[UPLOAD ERROR] Read failed at byte offset %d for %s: %v", chunk.ByteOffset, file.Name, err)
					...
					existingChunk, _ := e.db.FindCompletedChunkBySHA256(chunkHash)
					...
					if err != nil {
						atomic.AddInt32(&tracker.activeWorkers, -1)
						log.Printf("[UPLOAD ERROR] Encryption error on part %d: %v", chunk.ChunkIndex, err)
						errChan <- fmt.Errorf("lock error on part %d %w", chunk.ChunkIndex, err)
						return
					}

					pngWrapped := crypto.WrapInPNGContainer(encryptedPayload)
					...
					time.Sleep(time.Duration(150+workerID*25) * time.Millisecond)
					...
					if err != nil {
						...
						log.Printf("[UPLOAD ERROR] Discord rejected part %d on %s: %v", chunk.ChunkIndex+1, item.chName, err)
						errChan <- fmt.Errorf("upload error on part %d %w", chunk.ChunkIndex, err)
						return
					}

					_ = e.db.UpdateChunkStatusWithGuild(chunk.ID, db.StatusCompleted, item.guildID, res.ChannelID, res.MessageID, res.AttachmentID, res.AttachmentURL)
					_ = e.db.UpdateChunkHash(chunk.ID, chunkHash)
					_ = e.db.IncrementJobCompletedChunks(job.ID)
					...
					log.Printf("[UPLOAD PART] Part %d/%d (%.2f MB) done on %s (%.2f MB/s, ETA: %ds)", chunk.ChunkIndex+1, job.TotalChunks, float64(n)/(1024*1024), item.chName, speedMBs, etaSec)
	...
	if err, ok := <-errChan; ok && err != nil {
		_ = e.db.UpdateJobStatus(job.ID, db.JobStatusFailed)
		log.Printf("[UPLOAD FAILED] Job %s failed: %v", job.ID, err)
		e.broadcastTelemetry(TelemetryEvent{
			JobID:      job.ID,
			FileID:     file.ID,
			Status:     db.JobStatusFailed,
	...
	log.Printf("[UPLOAD COMPLETE] %s uploaded successfully (SHA256: %s)", file.Name, file.SHA256)
	...
	e.broadcastTelemetry(TelemetryEvent{
		...
		LogMessage:      fmt.Sprintf("File %s uploaded and verified across all storage shards!", file.Name),
	})
}

func (e *Engine) UploadDirectory(ctx context.Context, localDirPath, virtualParentPath string, targetGuildIDs ...string) ([]string, error) {
	e.mu.RLock()
	key := e.masterKey
	e.mu.RUnlock()

	if len(key) != crypto.KeyLength {
		return nil, errors.New("please enter your master password in settings first")
	}
	...
	dirRec := &db.FileRecord{
		...
		ParentID:  "",
		Name:      folderName,
		Path:      filepath.ToSlash(filepath.Join(virtualParentPath, folderName)),
		Size:      0,
		IsDir:     true,
	...
	err := filepath.Walk(normDir, func(path string, info os.FileInfo, err error) error {
		...
		jobID, err := e.UploadFile(ctx, path, vPath, dirRec.ID, targetGuildIDs...)
		if err != nil {
