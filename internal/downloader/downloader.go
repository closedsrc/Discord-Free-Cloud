...
func (e *Engine) broadcastTelemetry(event uploader.TelemetryEvent) {
	e.mu.RLock()
...
func (e *Engine) fetchChunkBytes(ctx context.Context, fileID string, chunk db.ChunkRecord, key []byte) ([]byte, error) {
...
func (e *Engine) fetchSingleChunk(ctx context.Context, fileID string, chunk db.ChunkRecord, key []byte) ([]byte, error) {
	...
	if chunk.AttachmentURL != "" {
		encBytes, err = e.discord.DownloadChunkBytes(ctx, chunk.AttachmentURL)
	}

	if (err != nil || len(encBytes) == 0) && chunk.ChannelID != "" && chunk.MessageID != "" {
		freshURL, freshErr := e.discord.GetFreshAttachmentURL(ctx, chunk.ChannelID, chunk.MessageID, chunk.AttachmentID)
		if freshErr == nil && freshURL != "" {
			encBytes, err = e.discord.DownloadChunkBytes(ctx, freshURL)
			if err == nil && len(encBytes) > 0 {
				_ = e.db.UpdateChunkStatusWithGuild(chunk.ID, db.StatusCompleted, chunk.GuildID, chunk.ChannelID, chunk.MessageID, chunk.AttachmentID, freshURL)
			}
		}
	}
	...
	rawPayload := crypto.UnwrapPNGContainer(encBytes)
	...
	if chunk.SHA256 != "" {
		if origs, err := e.db.FindAllCompletedChunksBySHA256(chunk.SHA256); err == nil {
			for _, o := range origs {
...
func (e *Engine) StreamDownloadRange(ctx context.Context, fileID string, start, end int64, w io.Writer) error {
	...
	if cached := e.getCachedChunk(cacheKey0); cached != nil {
		...
		dec, err := e.fetchChunkBytes(ctx, fileRec.ID, firstChunk, key)
		if err != nil {
			return err
		}
		firstDec = dec
		...
		if nextChunk, ok := chunkMap[firstChunk.ChunkIndex+1]; ok {
			...
			if e.getCachedChunk(nextCacheKey) == nil {
				go func() {
					...
					defer cancel()
					_, _ = e.fetchChunkBytes(bgCtx, fileRec.ID, nextChunk, key)
				}()
			}
		}
		return nil
			...
			for {
				...
				case ch, ok := <-chunkChan:
					...
					if cached := e.getCachedChunk(cacheKey); cached != nil {
						...
						dec, err := e.fetchChunkBytes(ctx, fileRec.ID, ch, key)
						if err != nil {
							mapMu.Lock()
							workerErr = err
							cond.Broadcast()
...
func (e *Engine) StreamDownload(ctx context.Context, fileID string, w io.Writer) error {
	...
	if fileRec.IsDir {
		return e.StreamFolderZip(ctx, fileID, w)
	}
	...
	log.Printf("[DOWNLOAD START] %s | Size: %.2f MB | Parts: %d", fileRec.Name, float64(fileRec.Size)/(1024*1024), len(chunks))

	if len(chunks) == 1 {
		decryptedBytes, err := e.fetchChunkBytes(ctx, fileRec.ID, chunks[0], key)
		if err != nil {
			log.Printf("[DOWNLOAD ERROR] Single part download failed for %s: %v", fileRec.Name, err)
			return err
		}
		_, err = w.Write(decryptedBytes)
		if err == nil {
			log.Printf("[DOWNLOAD COMPLETE] %s sent to browser successfully", fileRec.Name)
		}
		return err
	}
		...
		go func() {
			...
			for {
				...
				case ch, ok := <-chunkChan:
					...
					decryptedBytes, err := e.fetchChunkBytes(ctx, fileRec.ID, ch, key)
					if err != nil {
						mapMu.Lock()
						workerErr = err
						cond.Broadcast()
						mapMu.Unlock()
						log.Printf("[DOWNLOAD ERROR] Part %d fetch failed for %s: %v", ch.ChunkIndex+1, fileRec.Name, err)
						return
					}
		...
		for {
			...
			if data, ok := resMap[i]; ok {
				...
				if _, err := w.Write(data); err != nil {
					log.Printf("[DOWNLOAD ERROR] Browser write interrupted on part %d: %v", i+1, err)
					return fmt.Errorf("browser write error on part %d %w", i, err)
				}
				log.Printf("[DOWNLOAD PART] Part %d/%d (%.2f MB) sent to browser", i+1, len(chunks), float64(len(data))/(1024*1024))
				mapMu.Lock()
				delete(resMap, i)
				mapMu.Unlock()
				break
	...
	wg.Wait()
	log.Printf("[DOWNLOAD COMPLETE] %s finished streaming to browser", fileRec.Name)
	return nil
}

func (e *Engine) StreamFolderZip(ctx context.Context, folderID string, w io.Writer) error {
...
func (e *Engine) DownloadFile(ctx context.Context, fileID, localDestPath string) (string, error) {
	...
	go e.runDownloadPipeline(jobCtx, tracker, jobRec, fileRec, chunks, localDestPath, key)

	return jobID, nil
}

func (e *Engine) runDownloadPipeline(ctx context.Context, tracker *activeDownload, job *db.JobRecord, file *db.FileRecord, chunks []db.ChunkRecord, localDestPath string, key []byte) {
	defer func() {
		e.mu.Lock()
	...
	if err != nil {
		...
		e.broadcastTelemetry(uploader.TelemetryEvent{
			JobID:   job.ID,
			FileID:  file.ID,
		...
		go func(workerID int) {
			...
			for {
				...
				case chunk, ok := <-chunkChan:
					...
					decryptedBytes, err := e.fetchChunkBytes(ctx, file.ID, chunk, key)
					if err != nil {
						atomic.AddInt32(&tracker.activeWorkers, -1)
						errChan <- err
						return
