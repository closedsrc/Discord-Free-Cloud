...
func New(database *db.Database, discordClient *discord.Client, masterKey []byte) *Manager {
	return &Manager{
		db:        database,
...
func (s *Manager) SetMasterKey(key []byte) {
	s.masterKey = key
}

func (s *Manager) Sync(ctx context.Context) (string, error) {
	...
	if err != nil || metaChannel == nil {
		...
		if guildID != "" {
			res, err := s.discord.SetupServer(ctx, guildID)
	...
	aad := []byte("discord-free-cloud-backup-v1")
	...
	pngWrapped := crypto.WrapPNG(encryptedManifest)
...
func (s *Manager) Restore(ctx context.Context, metaChannelID string) (*Snapshot, error) {
	if len(s.masterKey) != crypto.KeyLength {
	...
	freshURL, err := s.discord.AttachmentURL(ctx, metaChannelID, "", "")
	if err != nil {
		return nil, fmt.Errorf("could not find backup in discord channel %w", err)
	}

	encBytes, err := s.discord.DownloadChunk(ctx, freshURL)
	if err != nil {
		return nil, fmt.Errorf("could not download backup file %w", err)
	}

	unwrapped := crypto.UnwrapPNG(encBytes)
	decryptedJSON, err := crypto.DecryptChunk(unwrapped, s.masterKey, []byte("discord-free-cloud-backup-v1"))
	if err != nil {
		decryptedJSON, err = crypto.DecryptChunk(unwrapped, s.masterKey, []byte("discord drive backup checkpoint v1"))
	}
	if err != nil {
